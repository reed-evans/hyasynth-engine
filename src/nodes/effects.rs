...
/// Mixer node that sums all input channels.
///
/// This is used for the master bus and other summing points.
...
impl Node for MixerNode {
    ...
    fn process(
        ...
        // Sum all inputs into output
        for input in inputs {
            for ch in 0..self.channels.min(input.channels) {
                let in_channel = input.channel(ch);
                let out_channel = output.channel_mut(ch);
                
                for i in 0..ctx.frames.min(in_channel.len()) {
                    out_channel[i] += in_channel[i];
                }
            }
        }

        // Check if actually silent
